...
class CaptchaSolver:
    ...
    def _solve_with_config(
        ...
        if not api_key:
            logger.warn(f"验证码识别供应商 {provider} 未配置 API Key，跳过第三方识别")
            return ""
    ...
    def _create_and_resolve(
        ...
        if not create_url:
            logger.error(f"验证码识别供应商 {provider} 缺少 API 地址配置")
            return ""
        ...
        if cls._has_error(response):
            logger.error(f"验证码识别供应商 {provider} 创建任务失败：{cls._format_error(response)}")
            return ""
        ...
        if not task_id or not get_url:
            logger.error(f"验证码识别供应商 {provider} 未返回可用结果")
            return ""
        ...
        while time.time() - started_at < timeout:
            ...
            if cls._has_error(result):
                logger.error(f"验证码识别供应商 {provider} 查询结果失败：{cls._format_error(result)}")
                return ""
            ...
            if text:
                return text
            logger.error(f"验证码识别供应商 {provider} 未返回识别文本：{result}")
            return ""

        logger.error(f"验证码识别供应商 {provider} 超时，未在 {timeout} 秒内返回结果")
        return ""
    ...
    def _post_json(url: str, payload: dict, timeout: int) -> Optional[dict]:
        ...
        try:
            ...
            logger.error(f"验证码识别响应解析失败：{err}")
            return None
